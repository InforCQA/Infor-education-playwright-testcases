...
const vendorContext = JSON.parse(JSON.stringify(require("../data/ReviewChangesToVendorGroupSetup.properties.json")));


export default function TCEDU_FSMReviewChangesToVendorGroupSetup() {
  
    describeTest('TCEDU_FSMReviewChangesToVendorGroupSetup', () => {
        ...
        test('Review changes to vendor group setup', async ({ }) => {
            await CloudSuite.navigateToApplication(ProductNames.FSM);

            // Collapse the context apps
		    await CloudSuite.collapseContextApps();

            await PayablesManagerFunctions.reviewChangesToVendorGroupSetup(vendorContext);
        });
