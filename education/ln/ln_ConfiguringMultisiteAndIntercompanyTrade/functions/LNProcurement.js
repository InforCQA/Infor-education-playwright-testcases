...
class LNProcurement extends BaseClass{
    ...
    static async createAPurchaseOrderForExternalMaterialDelivery(warehouseCnxt) {
		...
		log.info(`=========>>>>> The purchase order number is ${warehouseCnxt.purchaseOrderNum}`);

		await this.screenshot("Create purchase order (by distribution center)");
		...
		await this.screenshot("Review intercompany trade order - sales (by purchase office at distribution center)");
		...
		log.info("=========>>>>> Receive the purchase order (by your sales center) started <<<<<=========");
		...
		await this.screenshot("Receive the purchase order (by your sales center)");

		// Verifying the Status is closed
		await LNCommon.verifyValueWithLabel(OrderIntakeWorkbench_Lbl.STATUS, LNSessionCodes.PURCHASE_ORDER, toolbar.CLOSED);
		...
		await this.screenshot("Review the intercompany trade order transaction line (by distribution center)");
