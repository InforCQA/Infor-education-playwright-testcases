...
import Addresses_Lbl from '../constants/elementLbls/Addresses_Lbl';
import Addresses_Id from '../constants/elementIds/Addresses_Id';
import EnterpriseUnits_Lbl from '../constants/elementLbls/EnterpriseUnits_Lbl';
...
import CalculateStandardCostLbl from '../constants/elementLbls/CalculateStandardCostLbl ';
import CalculateStandardCost_Id from '../constants/elementIds/CalculateStandardCost_Id ';
import ItemDefaults_Id from '../constants/elementIds/ItemDefaults_Id';
import ItemDefaults_Lbl from '../constants/elementLbls/ItemDefaults_Lbl';
...
class LNMasterData extends BaseClass {
	...
	* -----------------------------------------------------------------------------------------*/
    static async reviewEnterpriseModelInLNCE(enterpriseMdlCnxt) {
        log.info('=========>>>>> Review the enterprise model in LN CE started <<<<<=========');
        ...
        expect(await emwPg.pushPins().count()).toBeGreaterThan(parseInt(toolbar.ZERO));
        ...
        log.info('=========>>>>> Review the enterprise model in LN CE completed <<<<<=========');
    }
    /*---------------------------------------------------------------------------------------
	...
	* -----------------------------------------------------------------------------------------*/
    static async reviewASiteInLNCE(enterpriseMdlCnxt) {
        log.info('=========>>>>> Review a site in LN CE started <<<<<=========');
        ...
        expect(departments).toBe(Number(toolbar.RECORD_TWO));
        ...
        for (let i = 0; i < settingsLbl.length; i++) {
            ...
            await expect(async () => {
                ...
                )).getAttribute(ElementAttributes.CLASS)).toContain(toolbar.CHECKED);
       ...
       for (const { tab, label, id } of items) {
        ...
        await LNCommon.closeTab(LNSessionTabs.PROCUREMENT_SETTINGS_BY_SITE); 
        ...
        await LNCommon.closeTab(LNSessionTabs.SALES_SETTINGS_BY_SITE);
       ...
       await LNCommon.verifySessionTab(LNSessionTabs.WAREHOUSING_SETTINGS_BY_SITE);
        ...
        await LNCommon.closeTab(LNSessionTabs.WAREHOUSING_SETTINGS_BY_SITE);
         await LNCommon.verifySessionTab(LNSessionTabs.SITE);
       ...
       await (await lnPg.closeButton(LNSessionCodes.SALES_OFFICES)).dblclick();
        ...
        await LNCommon.closeTab(LNSessionTabs.INTERCOMPANY_TRADE_AGREEMENT_TRANSFER_PRICING_RULES);
        ...
        await LNCommon.closeTab(LNSessionTabs.SITE);
        await LNCommon.closeTab(LNSessionTabs.ENTERPRISE_MODEL_WORKBENCH);

        await LNCommon.collapseLNModule(LNSessionTabs.MASTER_DATA);
        log.info('=========>>>>> Review a site in LN CE completed <<<<<=========');
    }
    /*-------------------------------------------------------------------------------------
    ...
    static async  createAnAddressTypeTheStep(structureCnxt) {
        log.info('=========>>>>> Create an address Type the step started <<<<<=========');
        ...
        log.info('=========>>>>> Create an address Type the step completed <<<<<=========');
    }
    /*-------------------------------------------------------------------------------------
    ...
    log.info('=========>>>>> Create an enterprise unit started <<<<<=========');
    ...
    if (!isPresent) {
        ...
        // Go to Details
        await LNCommon.drilldownRequiredRecord(LNSessionCodes.ENTERPRISE_UNITS,toolbar.FIRST_RECORD);

        // Handle pop-up
        await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.NO_BUSINESS_PARTNER_ASSIGNED_TO_ENTERPRISE_UNIT.replace('%s', structureCnxt.enterpriseUnit),toolbar.OK);
    ...
    log.info(`=========>>>>> The Enterprise Unit is ${createdUnit} <<<<<=========`);

    await LNCommon.clickMainMenuItem(LNSessionCodes.ENTERPRISE_UNITS, LNMenuActions_Id.SAVE_AND_EXIT);

    await LNCommon.collapseLNModule(LNSessionTabs.MASTER_DATA);

    log.info('=========>>>>> Create an enterprise unit completed successfully <<<<<=========');
}
 /*-------------------------------------------------------------------------------------
...
static async createPlanningCluster(planningCluster, planningClusterDesc) {
    log.info("=========>>>>> Create a planning cluster started <<<<<=========");
    ...
    log.info(`=========>>>>> The Planning cluster is ${planningCluster} <<<<<=========`);
    ...
    log.info("=========>>>>> Create a planning cluster completed <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async createSite(structureCnxt) {
  log.info("=========>>>>> Create a site started <<<<<=========");
  ...
  await LNCommon.filterRequiredRecord( Items_Lbl.SITE_GRID,Sites_Id.SITE_GRID,LNSessionCodes.SITES,structureCnxt.site);
  ...
  log.info("=========>>>>> The Site code is " + siteCode + " <<<<<=========");
  ...
  const warehousingElement = await lnPg.hyperlinkText(Sites_Lbl.WAREHOUSING_RDN,Sites_Id.WAREHOUSING_RDN,LNSessionCodes.SITE);
    ...
    try {
        ...
        log.info('Checked class not found');
    }
  ...
  if (!warehousingClass.includes(toolbar.CHECKED)) {
  ...
  const classFinal = await warehousingElement.getAttribute(ElementAttributes.CLASS);
  expect(classFinal).toContain(toolbar.CHECKED);
  ...
  log.info("=========>>>>> Create a site completed successfully <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async createSalesOfficeAndWarehouse(structureCnxt) {
  ...
  log.info(">>>>>>> Create a sales office and warehouse started <<<<<<<<");
  ...
  log.info(">>>>>>> Create a sales office and warehouse completed <<<<<<<<");
}
/*-------------------------------------------------------------------------------------
...
static async createSalesSettingBySite(structureCnxt) {
 log.info("=========>>>>> Create a sales setting by site started <<<<<=========");
  ...
  if (!salesClass.includes(toolbar.CHECKED)) {
    // Click Sales button
    await (await lnPg.hyperlinkText(Sites_Lbl.SALES_RDN,Sites_Id.SALES_RDN,LNSessionCodes.SITE)).click();

    // Handle confirmation popup
    await LNCommon.validateMessageAndHandlePopUp(`Sales settings are not present for site ${structureCnxt.site}. Do you want to create them?`,toolbar.YES);
  ...
  expect(finalClass).toContain(toolbar.CHECKED);
 ...
 log.info("=========>>>>> Create a sales setting by site completed <<<<<=========");
}
...
static async reviewNewSiteDetailsInEnterpriseModelWorkbench(structureCnxt) {
    ...
    log.info(
        "=========>>>>> Review the new site details in the Enterprise Model Workbench started <<<<<=========");
    ...
    log.info(
        "=========>>>>> Review the new site details in the Enterprise Model Workbench completed successfully <<<<<=========");
}
    ...
    static async reviewItemBySiteAndUpdatePurchasePrice(itemsBySiteCnxt) {
        ...
        log.info("=========>>>>> Review item by site and update purchase price started <<<<<=========");
		...
		await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEMS_BY_SITE, toolbar.FIRST_RECORD);
       ...
       await LNCommon.clickTextMenuItem(LNSessionCodes.ITEMS_PURCHASE_BY_SITE, LNMenuActions_Id.OK, LNMenuActions_Lbl.OK);
		...
		await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEM_DATA_WAREHOUSE, toolbar.FIRST_RECORD);
        ...
        log.info("=========>>>>> Review item by site and update purchase price completed <<<<<=========");
    }
    ...
    static async calculateStandardCostPerEnterpriseUnitSalesCenter(itemsBySiteCnxt) {
        ...
        log.info('=== Calculate standard cost per enterprise unit started ===');
        ...
        await LNCommon.triggerInputField(await LNCommon.getTextField(CalculateStandardCostLbl.FROM_ITEM,
        ...
        await expect(async () => {
            expect(await (await LNCommon.getTextField(CalculateStandardCostLbl.TO_ITEM, CalculateStandardCost_Id.TO_ITEM_SEG2,
                LNSessionCodes.CALCULATE_STANDARD_COST)).inputValue()).toBe(itemsBySiteCnxt.item[0]);
        }).toPass({ timeout: 10000 });

        await LNCommon.selectCheckbox(CalculateStandardCostLbl.ACTUALIZE_STANDARD_COST_AND_REVALUE_INVENTORY_CHK,
        ...
        await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.PROCESS_READY, toolbar.OK);
        ...
        await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEM_COSTINGS, toolbar.FIRST_RECORD);
        ...
        log.info('=== Calculate standard cost completed ===');
    }

    
    static async reviewItemBySalesOfficeAndUpdateSalesPrice(itemsBySiteCnxt) {

        log.info('=== Start reviewItemBySalesOfficeAndUpdateSalesPrice ===');
        ...
        await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEMS_SALES_BY_OFFICE, toolbar.FIRST_RECORD);
        await LNCommon.verifySessionTab(LNSessionTabs.ITEMS_SALES_BY_OFFICE);
        ...
        await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.DEFAULT_PRICE_BOOK, toolbar.OK);
        ...
        log.info('=== Completed reviewItemBySalesOfficeAndUpdateSalesPrice ===');
    }
    /*-------------------------------------------------------------------------------------
    ...
    static async reviewItemDefaults(itemCnxt) {
        ...
        await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEM_DEFAULTS, toolbar.FIRST_RECORD);
        ...
        await expect(async () => {
            expect(await (await lnPg.dropdownValueLabel(ItemDefaults_Lbl.ITEM_TYPE_DRP,
                ItemDefaults_Id.ITEM_TYPE_DRP)).innerText())
                .toBe(itemCnxt.itemType);

        }).toPass({ timeout: 10000 });
     
        // Verifying the Item Group value
        expect(await (await LNCommon.getTextField(ItemDefaults_Lbl.ITEM_GROUP_GRID, ItemDefaults_Id.ITEM_GROUP_GRID,
            LNSessionCodes.ITEM_DEFAULTS_DETAIL)).inputValue()).toBe(itemCnxt.itemGroup);

        await (await lnPg.verifyHeader(ItemDefaults_Lbl.SUBENTITIES)).hover();
        ...
        for (let i = 0; i < itemCnxt.sites.length; i++) {
            ...
            await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEM_CONTROL_AND_DEFAULTS_BY_SITE, toolbar.FIRST_RECORD);
            ...
            expect(await (await LNCommon
                ...
                .inputValue()).toBe(itemCnxt.sites[i]);

            await LNCommon.closeTab(LNSessionTabs.ITEM_CONTROL_AND_DEFAULTS_BY_SITE);
        }
        ...
        await (await lnPg.hyperlinkText(ItemDefaults_Lbl.SALES_BTN, ItemDefaults_Id.SALES_BTN,
        ...
        expect(await (await lnPg.dropdownValueLabel(ItemDefaults_Lbl.ITEM_TYPE_IN_SALES_ZOOM_DRP,
            ItemDefaults_Id.ITEM_TYPE_IN_SALES_ZOOM_DRP)).innerText()).toBe(itemCnxt.itemType);
        expect(await (await LNCommon
            ...
            .inputValue()).toBe(itemCnxt.itemGroup);
        ...
        await (await lnPg.hyperlinkText(ItemDefaults_Lbl.ORDERING_BTN, ItemDefaults_Id.ORDERING_BTN,
            LNSessionCodes.ITEM_DEFAULTS_DETAIL)).click();
        ...
        // Verifying the Details in Item Sales Defaults window
        expect(await (await lnPg.dropdownValueLabel(ItemDefaults_Lbl.ITEM_TYPE_IN_ORDERING_ZOOM_DRP,
            ItemDefaults_Id.ITEM_TYPE_IN_ORDERING_ZOOM_DRP)).innerText()).toBe(itemCnxt.itemType);
        expect(await (await LNCommon
        ...
        await (await lnPg.hyperlinkText(ItemDefaults_Lbl.PURCHASE_BTN, ItemDefaults_Id.PURCHASE_BTN,
            LNSessionCodes.ITEM_DEFAULTS_DETAIL)).click();
        ...
        expect(await this.isElementPresent(await lnPg.verifyHeader(ItemDefaults_Lbl.ORDER_DETAILS_SECTION))).toBeTruthy();
        ...
        await (await lnPg.hyperlinkText(ItemDefaults_Lbl.WAREHOUSING_BTN, ItemDefaults_Id.WAREHOUSING_BTN,
            LNSessionCodes.ITEM_DEFAULTS_DETAIL)).click();
        ...
        expect(await (await lnPg.dropdownValueLabel(ItemDefaults_Lbl.ITEM_TYPE_IN_WAREHOUSING_ZOOM_DRP, ItemDefaults_Id.ITEM_TYPE_IN_WAREHOUSING_ZOOM_DRP))
            .innerText()).toBe(itemCnxt.itemType);

        expect(await (await LNCommon.getTextField(ItemDefaults_Lbl.ITEM_GROUP_IN_WAREHOUSING_ZOOM,
        ...
        await LNCommon.closeTab(LNSessionTabs.ITEM_DEFAULTS);
        await LNCommon.closeTab(LNSessionTabs.ITEM_DEFAULTS);
    ...
    static async createANewItem(itemCnxt) {
		...
		if (await LNCommon.isRequiredRowPresent(LNSessionCodes.ITEMS, Items_Lbl.ITEM_GRID, Items_Id.ITEM_SEG_2_GRID,
			...
			await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.DELETE_SELECT_RECORD, toolbar.YES);
			
            await this.page.waitForLoadState();
            await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.DELETED_RECORD, toolbar.OK);
		}
		...
		await LNCommon.triggerInputField(await LNCommon.getTextField(Items_Lbl.ITEM, Items_Id.ITEM_SEC, LNSessionCodes.ITEM),
				itemCnxt.item);
        ...
        await this.pause(1);
        await (await LNCommon.getTextboxLookUpIcon(Items_Lbl.ITEM_GROUP, Items_Id.ITEM_GROUP, LNSessionCodes.ITEM)).waitFor({ state: 'visible', timeout: 5000 });
        await (await LNCommon.getTextboxLookUpIcon(Items_Lbl.ITEM_GROUP, Items_Id.ITEM_GROUP, LNSessionCodes.ITEM)).hover();
		await (await LNCommon.getTextboxLookUpIcon(Items_Lbl.ITEM_GROUP, Items_Id.ITEM_GROUP, LNSessionCodes.ITEM)).click();
		...
		await LNCommon.filterRequiredRecord(Items_Lbl.ITEM_GROUP_ZOOM_GRID, Items_Id.ITEM_GROUP_IN_ITEM_DEFAULTS_ZOOM_GRID,
				LNSessionCodes.ITEM_DEFAULTS, itemCnxt.itemGroup);
		await LNCommon.selectRecord(LNSessionCodes.ITEM_DEFAULTS, toolbar.FIRST_RECORD);
        ...
        await expect(async () => {
            ...
            await this.page.keyboard.press('Tab');
            expect(await (await LNCommon.getTextField(Items_Lbl.UNIT_SET, Items_Id.UNIT_SET, LNSessionCodes.ITEM))
                .inputValue(), "The Unit set field is empty").not.toBe("");
        ...
        await expect(async () => {
            ...
            expect(await (await LNCommon.getTextField(Items_Lbl.UNIT, Items_Id.UNIT, LNSessionCodes.ITEM))
                .inputValue()).not.toBe("");
		...
		await (await lnPg.hyperlinkText(Items_Lbl.PURCHASE_BTN, Items_Id.PURCHASE_BTN, LNSessionCodes.ITEM))
				.click();
		...
		await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.REPLACE_LATEST_PURCHASE_PRICE, toolbar.YES);

		// Verifying the Dialogbox title
		await LNCommon.verifyDialogBoxWindow(LNSessionTabs.ITEM_PURCHASE_TDIPUA);
        await this.pause(3);
		await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.REPLACE_AVERAGE_PURCHASE_PRICE_ACTUAL_PRICE, toolbar.YES);
		...
		await LNCommon.drilldownRequiredRecord(LNSessionCodes.ITEM_COSTING, toolbar.FIRST_RECORD);
...
static async reviewItemAtEnterpriseLevel(itemCnxt) {
  ...
  log.info("=========>>>>> Review the item at an enterprise level started <<<<<=========");
  ...
  log.info("=========>>>>> Review the item at an enterprise level completed successfully <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async reviewDomainSpecificData() {
   ...
   log.info("=========>>>>> Review the domain-specific data started <<<<<=========");
  ...
  expect(await salesLocator.getAttribute(ElementAttributes.CLASS)).toContain(toolbar.CHECKED);
  ...
  expect(await serviceLocator.getAttribute(ElementAttributes.CLASS)).not.toContain(toolbar.CHECKED);

   log.info("=========>>>>> Review the domain-specific data completed successfully <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async reviewPurchaseDataForItem(itemCnxt) {
   ...
   log.info("=========>>>>> Review Purchase Data for the Item started <<<<<=========");
  ...
  await LNCommon.verifySessionTab(LNSessionTabs.ITEM);
   ...
   log.info("=========>>>>> Review Purchase Data for the Item completed successfully <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async reviewSalesOfficeForItem() {
   log.info("=========>>>>> Review sales office for the item started <<<<<=========");
  ...
  // Close Item session
  await LNCommon.closeTab(LNSessionTabs.ITEM);

  // Confirm return to Items session
  await LNCommon.verifySessionTab(LNSessionTabs.ITEMS);

   log.info("=========>>>>> Review sales office for the item completed successfully <<<<<=========");
}
...
static async reviewStandardCostsByEnterpriseUnit(itemCnxt) {
   log.info("=========>>>>> Review standard costs by enterprise unit started <<<<<=========");
  ...
  for (let i = 0; i < itemCnxt.enterpriseUnits.length; i++) {
    ...
    await LNCommon.closeTab(LNSessionTabs.ITEM_COSTING);
    await LNCommon.verifySessionTab(LNSessionTabs.ITEM);
  }
  ...
  await LNCommon.closeTab(LNSessionTabs.ITEM);
  await LNCommon.closeTab(LNSessionTabs.ITEMS);
  await LNCommon.collapseLNModule(LNSessionTabs.MASTER_DATA);

   log.info("=========>>>>> Review standard costs by enterprise unit completed successfully <<<<<=========");
}
/*-------------------------------------------------------------------------------------
...
static async updateLocalBusinessPartnerCustomerData(businessCnxt) {
    log.info('=========>>>>> Update local business partner (customer) data started <<<<<=========');
    ...
    await LNCommon.drilldownRequiredRecord(LNSessionCodes.BUSINESS_PARTNERS,toolbar.FIRST_RECORD);
    ...
    await LNCommon.verifySessionTab(LNSessionTabs.SHIP_TO_BUSINESS_PARTNER_BY_SITE);
    // Screenshot and Save
    //await LNCommon.takeScreenshot('Update local business partner (customer) data');
    ...
    await LNCommon.collapseLNModule(LNSessionTabs.MASTER_DATA);
    log.info('=========>>>>> Update local business partner (customer) data completed successfully <<<<<=========');
  }
...
static async createProjectData(itemNo) {
  log.info("=========>>>>> Create Project Data started <<<<<=========");
  ...
  const subentitiesVisible = await this.isElementPresent(await commonPg.verifyHeader(Items_Lbl.SUBENTITIES));
  ...
  if (!(await this.isElementPresent(await commonPg.verifyHeader(Items_Lbl.PROJECT_PLANNING)))) {
    throw new Error(`${Items_Lbl.PROJECT_PLANNING} is not present`);
  }

  if (!(await this.isElementPresent(await commonPg.verifyHeader(Items_Lbl.UNIT_COST)))) {
  ...
  await this.screenshot("Create Project Data");

  // Collapse Master Data LN module
  await LNCommon.collapseLNModule(LNSessionTabs.MASTER_DATA);

  log.info("=========>>>>> Create Project Data completed successfully <<<<<=========");
}
}
