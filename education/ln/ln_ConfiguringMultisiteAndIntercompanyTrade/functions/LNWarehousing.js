...
class LNWarehousing extends BaseClass {
    ...
    static async createAWarehouseTransferForInternalMaterialDelivery(warehouseCnxt) {
		...
		let commonPg = new LNPage(this.page);
            ...
            await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.ENTER_A_VALUE_FOR_THE_ADDRESS_FIELD, toolbar.OK);
            ...
            await LNCommon.updateDefaultFilter(Orders_Id.WAREHOUSE_GRID, LNSessionCodes.WAREHOUSES, toolbar.CONTAINS);
            ...
            await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.NO_DISTANCE_HAS_BEEN_DEFINED_BETWEEN_ADDRESSES, toolbar.OK);
            ...
            expect(await LNCommon.getRequiredValueFromTheGrid(LNSessionCodes.OUTBOUND_LINES,
                ...
                Number(toolbar.FIRST_RECORD)), `The value Order line segment one is not ${warehouseCnxt.orderLine[0]}`)
            ...
            expect(await LNCommon.getRequiredValueFromTheGrid(LNSessionCodes.OUTBOUND_LINES,
                ...
                Number(toolbar.FIRST_RECORD)), `The value Order line segment two is not ${warehouseCnxt.orderLine[1]}`)
            ...
            await LNCommon.dataCellElement(
                ...
                Number(toolbar.FIRST_RECORD), warehouseCnxt.orderedQuantity);

            await LNCommon.clickMainMenuItem(LNSessionCodes.WAREHOUSING_ORDER, LNMenuActions_Id.SAVE);

            await this.screenshot("Create warehouse transfer (by sales center)");

            log.info("=========>>>>> Create warehouse transfer (by sales center) completed sucessfully <<<<<=========");
            ...
            log.info("=========>>>>> The Intercompany Trade number for Purchase is "
            ...
            // Verifying the Details in Tax, Pricing, Hours Expenses, Freight, Fianancial tab
            ...
            log.info("=========>>>>> Ship warehouse transfer (by distribution center) started <<<<<=========");
            ...
            await this.screenshot("Ship warehouse transfer (by distribution center)");
            ...
            await this.screenshot("Review the intercompany trade order transaction line (by sales center)");
    ...
    static async reviewIntercompanyTradeOrderSalesByDistributionCenter(warehouseCnxt) {
        ...
        try {
            ...
            await this.screenshot("Review intercompany trade order - sales (by distribution center)");
...
static async unhideProjectActivityFieldsAndCostComponentFieldsInWarehouseOrdersSession(orderNo, orderLine, personalizeColumns) {
  log.info("=========>>>>> Unhide Project activity fields and Cost Component fields in the warehouse orders session started <<<<<=========");
  ...
  await (await commonPg.personalizationOptions(LNSessionCodes.OUTBOUND_LINES, LNMenuActions_Lbl.PERSONALIZATION)).click();
  await this.page.waitForTimeout(1000);
  await (await commonPg.personalizationOptions(LNSessionCodes.OUTBOUND_LINES, LNMenuActions_Lbl.PERSONALIZE_FORM)).click();
  ...
  await (await commonPg.allTab()).click();
  await this.page.waitForTimeout(2000);
  ...
  for (const columnName of personalizeColumns) {
    ...
    await (await commonPg.allFilter()).fill(columnName);
    await this.page.keyboard.press('Tab');
    await this.page.waitForTimeout(2000);

    // Select checkbox for the personalize column if not selected
    const isCheckboxSelected = ((await (await commonPg.allCheckBox()).getAttribute(ElementAttributes.CLASS)).toLowerCase()).includes(toolbar.SELECTED.toLowerCase());
    if (!isCheckboxSelected) {
      await (await commonPg.allCheckBox()).click();
    }

    // Clear the Hidden checkbox if it is selected
    const isHiddenChecked = (await (await (await commonPg.hiddenCheckbox()).first()).getAttribute(ElementAttributes.CLASS)).includes(toolbar.CHECKED);
    if (isHiddenChecked) {
     await (await (await commonPg.hiddenCheckbox()).first()).click();
      await this.page.waitForTimeout(1000);
    }

    // Verify Hidden checkbox is cleared
    
    const checkboxClass = await (await (await commonPg.hiddenCheckbox()).first()).getAttribute(ElementAttributes.CLASS);
    expect(checkboxClass).not.toBe(toolbar.CHECKED);

  
    // Clear checkbox for the personalize column if it is selected
    const stillSelected = (((await (await commonPg.allCheckBox()).getAttribute(ElementAttributes.CLASS))).toLowerCase()).includes(toolbar.SELECTED.toLowerCase());
    if (stillSelected) {
      await (await commonPg.allCheckBox()).click();
    }
  }
  ...
  await (await commonPg.personalizationSave()).click();
  await this.page.waitForTimeout(2000);

  // Validate the popup message and click OK
  await LNCommon.validateMessageAndHandlePopUpIfExists(LNPopupMsg.PERSONALIZATION_SAVED, toolbar.OK);
  ...
  await this.screenshot("Unhide_Project_Activity_and_Cost_Component_Fields");

  // Close Warehousing Order tab
  await LNCommon.closeTab(LNSessionTabs.WAREHOUSING_ORDER);

  // Verify Warehousing Orders session tab
  await LNCommon.verifySessionTab(LNSessionTabs.WAREHOUSING_ORDERS);

  // Close Warehousing Orders tab
  await LNCommon.closeTab(LNSessionTabs.WAREHOUSING_ORDERS);

  // Collapse Warehousing LN module
  await LNCommon.collapseLNModule(LNSessionTabs.WAREHOUSING);

  log.info("=========>>>>> Unhide Project activity fields and Cost Component fields in the warehouse orders session completed successfully <<<<<=========");
}
/**
...
static async createWarehouseTransferBostonWarehouseToProject(warehouseCnxt) {
  log.info("=========>>>>> Create warehouse transfer (Boston Warehouse to Project PRJ000003) started <<<<<=========");
  ...
  expect(isShipFromVisible).toBeTruthy();

  // Click zoom icon in Ship From Code field (3rd segment)
  await (await LNCommon.getTextboxLookUpIcon(Orders_Lbl.CODE, Orders_Id.SHIP_FROM_CODE_SEGMENT_THREE, LNSessionCodes.WAREHOUSING_ORDER)).click();
  ...
  const isShipToVisible = await this.isElementPresent(await commonPg.verifyHeader(Orders_Lbl.SHIP_TO));
  expect(isShipToVisible).toBeTruthy();

  // Click zoom icon in Ship To Code field (3rd segment)
 await(await LNCommon.getTextboxLookUpIcon(Orders_Lbl.CODE_SHIP_TO, Orders_Id.CODE_SHIP_TO_THREE_SEG, LNSessionCodes.WAREHOUSING_ORDER)).click();
  ...
  await LNCommon.validateMessageAndHandlePopUp(LNPopupMsg.NO_DISTANCE_HAS_BEEN_DEFINED_BETWEEN_THE_ADDRESSES, toolbar.OK);
  ...
  const isOrderLinePresent = await LNCommon.isRequiredRowPresent(LNSessionCodes.OUTBOUND_LINES, Orders_Lbl.ORDER_LINE_IN_OUTBOUND_LINES_GRID, Orders_Id.ORDER_LINE_IN_OUTBOUND_LINES_GRID, warehouseCnxt.orderLine);
  expect(isOrderLinePresent).toBeTruthy();

  // Set Item field value for first row (index 0)
  await LNCommon.dataCellElement(await LNCommon.getDataCell(Orders_Lbl.ITEM_GRID, Orders_Id.ITEM_IN_OUTBOUND_LINES_SEGMENT_TWO_GRID, LNSessionCodes.OUTBOUND_LINES), 0, warehouseCnxt.item);

  // Set Ordered Quantity for first row (index 0)
  await LNCommon.dataCellElement(await LNCommon.getDataCell(Orders_Lbl.ORDERED_QUANTITY_GRID, Orders_Id.ORDERED_QUANTITY_GRID, LNSessionCodes.OUTBOUND_LINES), 0, String(warehouseCnxt.orderedQty));
  ...
  await(await LNCommon.getTextboxLookUpIconInGrid(Orders_Lbl.TO_ACTIVITY_GRID, Orders_Id.TO_ACTIVITY_GRID, LNSessionCodes.OUTBOUND_LINES)).click();

  // Verify Activities dialog
  await LNCommon.verifyDialogBoxWindow(LNSessionTabs.ACTIVITIES);

  // Filter and select activity
  await LNCommon.filterRequiredRecord(Orders_Lbl.ACTIVITY_ZOOM_GRID, Orders_Id.ACTIVITY_ZOOM_GRID, LNSessionCodes.ACTIVITIES_LINES, String(warehouseCnxt.activity));
  await LNCommon.selectRequiredRecord(LNSessionCodes.ACTIVITIES_LINES, Orders_Lbl.ACTIVITY_ZOOM_GRID, Orders_Id.ACTIVITY_ZOOM_GRID, String(warehouseCnxt.activity));
  ...
  await (await LNCommon.getTextboxLookUpIconInGrid(Orders_Lbl.TO_COST_COMPONENT_GRID, Orders_Id.TO_COST_COMPONENT_GRID, LNSessionCodes.OUTBOUND_LINES)).click();
  ...
  await LNCommon.filterRequiredRecord(Orders_Lbl.COST_COMPONENT_ZOOM_GRID, Orders_Id.COST_COMPONENT_ZOOM_GRID, LNSessionCodes.COST_COMPONENTS, String(warehouseCnxt.costComponent));
  await LNCommon.selectRequiredRecord(LNSessionCodes.COST_COMPONENTS, Orders_Lbl.COST_COMPONENT_ZOOM_GRID, Orders_Id.COST_COMPONENT_ZOOM_GRID, String(warehouseCnxt.costComponent));
  ...
  await LNCommon.selectRequiredRecord(LNSessionCodes.OUTBOUND_LINES, Orders_Lbl.ORDER_LINE_IN_OUTBOUND_LINES_GRID, Orders_Id.ORDER_LINE_IN_OUTBOUND_LINES_GRID, warehouseCnxt.orderLine);
  ...
  warehouseCnxt.intercompanyTradeNumSales = await (await LNCommon.getTextField(Orders_Lbl.ORDER, Orders_Id.ORDER_IN_INTERCOMPANY_TRADE_ORDER_SALES, LNSessionCodes.INTERCOMPANY_TRADE_ORDER_SALES)).inputValue();

  log.info(`=========>>>>> The Intercompany trade number for sales is ${warehouseCnxt.intercompanyTradeNumSales} <<<<<=========`);
  ...
  // Store Order number
  warehouseCnxt.order = await (await LNCommon.getTextField(Orders_Lbl.ORDER, Orders_Id.ORDER, LNSessionCodes.WAREHOUSING_ORDER)).inputValue();
  log.info(`=========>>>>> The Order number for sales is ${warehouseCnxt.order} <<<<<=========`);

  // Take screenshot
  await this.screenshot("Create_Warehouse_Transfer_Boston_Project.png");
  ...
  log.info("=========>>>>> Create warehouse transfer (Boston Warehouse to Project PRJ000003) completed successfully <<<<<=========");
}
...
static async shipWarehouseTransferByWarehouse(warehouseCnxt) {
  ...
  log.info("=========>>>>> Ship warehouse transfer (by warehouse) started <<<<<=========");
  ...
  const statusIsShipped = await commonPg.statusField(LNSessionCodes.WAREHOUSING_ORDER,Orders_Id.HEADER_STATUS, toolbar.SHIPPED);

  expect(statusIsShipped).toBeTruthy();

  // Screenshot for evidence
  await this.page.screenshot({ path: 'Ship_warehouse_transfer_by_warehouse.png', fullPage: true });

  log.info("=========>>>>> Ship warehouse transfer (by warehouse) completed successfully <<<<<=========");
}
...
static async reviewIntercompanyTradeOrderTransactionLineByProject(warehouseCnxt) {
  ...
  log.info("=========>>>>> Review the intercompany trade order transaction line (by Project) started <<<<<=========");
  ...
  const generalSectionPresent = await this.isElementPresent(await commonPg.verifyHeader(Orders_Lbl.GENERAL));
  ...
  const purchaseSectionPresent = await this.isElementPresent(await commonPg.verifyHeader(Orders_Lbl.PURCHASE));
  expect(purchaseSectionPresent).toBeTruthy();

  const salesSectionPresent = await this.isElementPresent(await commonPg.verifyHeader(Orders_Lbl.SALES));
  expect(salesSectionPresent).toBeTruthy();

  // Screenshot for documentation
  await this.screenshot({ path: 'Review_intercompany_trade_order_transaction_line_by_project.png', fullPage: true });
  ...
  log.info("=========>>>>> Review the intercompany trade order transaction line (by Project) completed successfully <<<<<=========");
}
}
