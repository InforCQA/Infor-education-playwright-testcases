...
import LNPage from "./LNCommonPage";
import toolbar from "./LNCommonConstants/ToolbarActions";
import LNSessionTabs from "./LNCommonConstants/LNSessionTabs";
import LNSessionCodes from "./LNCommonConstants/LNSessionCodes";
...
class LNCommon extends BaseClass {
    ...
    static async navigateToLNModule(module, ...subModules) {
            ...
            if (await this.isElementPresent(await lnPg.inforMainModules(module))) {
                ...
                log.info("=========>>>>> Navigated to '" + subModules[subModules.length - 1] + "' <<<<<=========");
            }
    }
    ...
    static async selectGridTab(tabName, sessionCode) {
        ...
        try {
            ...
            if (!classAttr.includes(toolbar.IS_ACTIVE)) {
    ...
    static async clickAndSelectDropdownFieldGridFilter(elementId, listItem, label, sessionCode) {
        ...
        if (currentValue.trim() != listItem) {
            ...
            for (let count = 0; count < 4; count++) {
                const classAttr = await (await lnPg.dropdownStsValue(elementId)).getAttribute(ElementAttributes.CLASS);
                if (!classAttr.includes(toolbar.IS_ACTIVE)) {
    ...
    static async selectRequiredRecord(sessionCode, columnName, elementId, value) {
        ...
        if (!classAttr.includes(toolbar.CHECKED)) {
            await checkbox.click();
        }

        log.info("=========>>>>> Selected row " + rowNo + " for " + columnName + " = " + value + " <<<<<=========");
    ...
    *--------------------------------------------------------------------------------*/
    ...
    static async getRequiredValueFromTheGrid(sessionCode, columnName, elementId, rowNum) {
        ...
        log.info("=========>>>>> Retreived " + fetchedValue + " for " + columnName + " from row - " + rowNum + " <<<<<=========");
    ...
    static async filterAndSelectFirstRecord(label, elementId, filterItem, sessionCode) {
        ...
        const firstRecord = await lnPg.selectRequiredRecord(sessionCode, toolbar.FIRST_RECORD);
        await firstRecord.click();

        const classAttr = await firstRecord.getAttribute(ElementAttributes.CLASS);
        expect(classAttr.includes(toolbar.CHECKED)).toBeTruthy();
    }
    /*--------------------------------------------------------------------------------------------
    ...
    static async dataCellElement(baseLocator, position, data) {
        ...
        log.info(`INFO: DataCell element is being used for ${await baseLocator.toString()}`);

        // Convert to 1-based index like in Selenium

        // Construct dynamic locator based on position
        const element = await baseLocator.nth(position); 
    ...
    *--------------------------------------------------*/
    static async selectCheckbox(label, elementId) {
        ...
        if (!(await classAttr.includes(toolbar.CHECKED))) {
    ...
    *------------------------------------------------------------------------------------------*/
    ...
    static async deselectCheckbox(label, elementId) {
        ...
        const checkbox = await lnPg.selectCheckboxLabel(label, elementId);
        await this.page.waitForTimeout(1000);
        classAttr = await checkbox.evaluate(el => el.className);
        expect(await classAttr.includes(toolbar.CHECKED)).toBeFalsy();
    }
    ...
    static async handleDevice() {
        ...
        await LNCommon.triggerInputField(await lnPg.device(), toolbar.DEVICE_D);
    ...
    static async selectRadioBtn(text, elementId) {
        ...
        await expect(async () => {
            ...
            expect((await radio.getAttribute(ElementAttributes.CLASS)).includes(toolbar.CHECKED), `radio button is not selected`)
                .toBeTruthy();
        }).toPass({ timeout: 90000 });
    }
// ----------------------------------------------------------------------
  ...
  if (isPresent) {
    ...
    for (let i = popupTexts.length - 1; i >= 0; i--) {
      if (popupTexts[i].toLowerCase().includes(popupText.toLowerCase())) {
        log.info(`=========>>>>> Pop Up message: ${popupTexts[i]} <<<<<=========`);
 ...
 */
    ...
    static async handleYesPopUp() {
        ...
        if (count > 0) {
            ...
            if (textCount > 0) {
                log.info("=========>>>>> Pop Up message: " + await (await lnPg.popupText.nth(0)).textContent() + " <<<<<=========");
            }
    ...
    static async handlePopUp() {
        ...
        if(count>0) {
            ...
            if(textCount>0){
                ...
                log.info("=========>>>>> Pop Up message: "+ await popupText.textContent() +" <<<<<=========");
            }
    ...
    static async runProgram(sessionID)
	{
		...
		{
			log.info("INFO : ========>>>>> LN Menu is Closed <<<<<=========");
			await (await lnPg.lnMenu()).click();
			log.info("INFO : ========>>>>> LN Menu is Opened <<<<<=========");
		}else
		{
			log.info("INFO : ========>>>>> LN Menu is Opened <<<<<=========");
		}
        ...
        if (await (await (await lnPg.optionsCaret()).getAttribute(ElementAttributes.ICONID))?.includes(toolbar.DOWN)) {
            await (await lnPg.lnOptions()).click();
        }

        const locator =await (await lnPg.iframe()).locator(`#${toolbar.NODE_OPTIONS_LABEL}`);
        ...
        await expect(async () => {
            ...
            await this.page.waitForTimeout(3000);
            await (await lnPg.lnRunprogramInput()).fill(sessionID);

        }).toPass({ timeout: 10000 });
	
		await this.page.keyboard.press('Tab');

		log.info("===="+await (await (await lnPg.sessionCode()).inputValue()).substring(0,13));
        
		if((await (await (await lnPg.sessionCode()).textContent()).substring(0,13).toLowerCase())===(LNSessionCodes.BILL_OF_MATERIAL.toLowerCase())) {
			
			if(!await (await (await lnPg.openInNewWindow()).getAttribute(ElementAttributes.CLASS))?.includes(toolbar.CHECKED)) {
		...
		log.info("INFO : ========>>>>> Navigated to "+sessionID+" Session <<<<<=========");
	}
    ...
    static async selectRecord(sessionCode, recordNum) {
        ...
        if (!await (await (await lnPg.selectRequiredRecord(sessionCode, recordNum)).getAttribute(ElementAttributes.CLASS)).includes(toolbar.CHECKED)) {
        ...
        await expect(async () => {
            ...
            expect(await (await (await lnPg.selectRequiredRecord(sessionCode, recordNum)).getAttribute(ElementAttributes.CLASS)).includes(toolbar.CHECKED), " checkbox is not selected").toBeTruthy();
    ...
    static async clickRowInDialogBox(id, ...parameters) {
        ...
        const commonPg = initElements(FSMCommonPage);

        // Collapse toggle menu
        await collapseToggleMenu();
        ...
        for (let i = 0; i < length; i++) {
            const col = await getDynamicElement(commonPg.thColumn, columnNames[i], id);
            columnIDs[i] = await col.getAttribute(ElementAttributes.ID);
        }
        ...
        switch (length) {
            ...
            case 1:
                ...
                while (!(await CommonFunctions.isDynamicElementPresent(commonPg.row, columnIDs[0], values[0]))) {

                    if (await CommonFunctions.isDynamicElementPresent(commonPg.btnNextWithId, id)) {

                        const nextBtn = await getDynamicElement(commonPg.btnNextWithId, id);
                        const nextClass = await nextBtn.getAttribute(ElementAttributes.CLASS);

                        if (!nextClass.includes(FSMCommons.IS_DISABLED)) {
                            await nextBtn.click();
                ...
                // Required row
                requiredRow = await getDynamicElement(commonPg.row, columnIDs[0], values[0]);
                break;
            ...
            case 2:
                ...
                while (!(await CommonFunctions.isDynamicElementPresent(
                    commonPg.rowForTwoCols,
                    columnIDs[0], values[0],
                    columnIDs[1], values[1],
                    columnIDs[0], values[0],
                    columnIDs[1], values[1]
                ))) {
                ...
                // Required row
                requiredRow = await getDynamicElement(
                    commonPg.rowForTwoCols,
                    columnIDs[0], values[0],
                    columnIDs[1], values[1],
                    columnIDs[0], values[0],
        ...
        // Get row index
        const rowNo = parseInt(await requiredRow.getAttribute(ElementAttributes.ARIA_ROWINDEX));

        // Click on row
        await pause(1);
        await (await actions()).moveTo(requiredRow).click().perform();
        await pause(1);
