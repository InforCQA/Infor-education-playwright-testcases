...
class CloudSuite extends BaseClass {
    ...
    static async navigateToApplication(appName) {
        ...
        if (!await this.isDynamicElementPresent(homePg.appNameV2, ProductNames.DOCUMENT_MANAGEMENT)) {
            element = await this.getDynamicElement(homePg.seeMore, Commons.APPLICATIONS);
            await element.click({ force: true });
        }
        element = await this.getDynamicElement(homePg.appNameV2, ProductNames.DOCUMENT_MANAGEMENT);
        await element.click();
    }
}
