...
class LNSales extends BaseClass {
  ...
  * ------------------------------------------------------------------------------------*/
  ...
  static async createSalesOrderAndReviewIntercompanyTradeOrderPurchaseBySalesCenter(businessCnxt) {
    ...
    console.log("=========>>>>> Review intercompany trade order - purchase (by the sales center) started <<<<<=========");
    ...
    console.log("=========>>>>> Review intercompany trade order - purchase completed <<<<<=========");

    // 3.1.3
    await LNCommonFunctions.reviewIntercompanyTradeOrderSalesByTheDistributionCenter(businessCnxt);
    
    // Part 3: Ship Sales Order
    console.log("=========>>>>> Ship sales order (by the distribution center) started <<<<<=========");
    ...
    await expect(async () => {
      expect(await this.isElementPresent(await commonPg.status(OrderIntakeWorkbench_Lbl.STATUS, LNSessionCodes.SALES_ORDER, LNCommons.IN_PROCESS
      ))).toBeTruthy();
    ...
    await expect(async () => {
      ...
      )).inputValue()).toBe(LNCommons.SHIPPED);
    }).toPass({ timeout: 60000 });
    ...
    console.log("=========>>>>> Ship sales order completed <<<<<=========");

    // Part 4: Review Transaction Line
    console.log("=========>>>>> Review transaction line (by sales center) started <<<<<=========");
    ...
    console.log("=========>>>>> Review intercompany trade order transaction line completed successfully <<<<<=========");
  }
}
